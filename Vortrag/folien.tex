...
\visible<3->{
$$
\nabla \cdot  \vec v_i = 0 \text{  in } \Omega_i
$$
$$
\frac{\partial \vec v_i}{\partial t} + \left(\vec  v _i\cdot \nabla \right) \vec v_i
=
-\frac{1}{\varrho_i}\nabla p_i + \nu_i \nabla^2 \vec v_i \text{  in } \Omega_i
$$
}
\end{frame}
...
\begin{figure}[h!]
    %\flushright
\includegraphics[width=3cm]{tutorial_basics03_d2q9.png}
\end{figure}
...
\visible<1->{Interface between fluid phases is captured by level set method}
\visible<2->{
\begin{itemize}
\item<2-> Level set function: $\varphi:= \varphi(x,t) \rightarrow \mathbb{R}$

Interface is the zero-isosurface of this function
\item<3-> Level set equation:
