%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FilmFlowLinear.m: Planar film low test case
%  using linearly interpolated interface condition
%  here we can prescribe an arbitrary (non-integral)
%  film thickness
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
thickness = 8.0
constant_delta = thickness - (floor(thickness+0.5)-0.5) % -0.5 because of cell centers

lx = 8; 
ly = floor(thickness+0.5)+2; % include 2 buffer nodes for doing bc stuff
tau = 1.0;
omega = 1/tau;
fx = +1e-5;
fy = -0.00000;
...
nu = (2*tau-1)/6
h = ly-2; % 2 buffer nodes
js = [1:1:h].-0.5 % "real world" coordinates
uIdeal = fx/nu*(thickness.-0.5*js).*js
...
for t=1:3500
   ...
   [rho, ux, uy] = calcMacro(f);
    ...
    subplot(2,1,2);
    plot([2:(ly-1)], (rho(2:ly-1,floor(lx/2))-1)/3.0);
    (rho(2:ly-1,floor(lx/2))-1)/3.0
    ...
    plot( [1.5 js.+1.5 ly-0.5], [0 uIdeal fx/nu*(thickness.-0.5*(h)).*(h) ], ...
   ...
   f = interfaceLiBeforeStream(f, neq, interface, constant_delta, lambda_even, ones(ly,lx), ux, uy);
