...
DEFUN_DLD( cellConversion3,args,, "do necessary cell conversions")
{
   ...
   for(int i = 0; i < ly; i++){
      ...
      if ( (fill(i,j)>= 1.0 + tol) && (interface(i,j)== 1) )  	// convert cell to liquid cell 
      {
	   ...
	   if(nj < 0){
   	   ...
   	   if( (gas(ni,nj) == 1) || (conv_I_L(ni,nj) ==1) )	// search for gas cells! -> convert them to interface cells
   	   {
             ...
             //std::cout << "Generate new interface cells: ni= " << ni << "nj: " << nj << std::endl;
         ...
         if(counter != 0)     
	 {
          ...
          ///////////////////////////////////////////////////
  	  std::cout << "G -> I begin!!! " << std::endl;
          std::cout << "start initialize process!!! " << std::endl;
          // initialize the new interface cells!!!
          double rho         = 0.0;
          double ux          = 0.0;
          double uy          = 0.0;
          int    counter_rho = 0;
	...
	if(counter != 0)
      ...
      {
	 std::cout << "L -> I begin!!!  i:= " << i << "j:= " << j << std::endl;
         interface(i,j) = 1;
         liquid(   i,j) = 0;
         std::cout << " end of conversion: L -> I! " << std::endl;
      ...
      {
	std::cout << "G -> I begin!!! i:= " << i << "j:= " << j<< std::endl;
   	interface(i,j) = 1;
   	gas(i,j)       = 0;
