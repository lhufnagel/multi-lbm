...
\title{Implementation of a lattice Boltzman method for immiscible multiphase flow simulations using the level set method} 
...
\author
{Lorenz Hufnagel,$^{1}$ Daniel Zint,$^{1}$\\
\\
\normalsize{$^{1}$Chair for System-Simulation, Friedrich Alexander Univerit\"at Erlangen-N\"urnberg}\\
\normalsize{91058 Erlangen, Germany}\\ % ist das so richtig?
\\
...
\begin{sciabstract}
  We implemented the lattice Boltzmann method for immiscible multiphase flow simulations and combined it with the level set method according to the paper of G. Thömmes FUSSNOTE. The level set method is used to calculate the movement of the interface between the two phases. The input for the level set method is computed by the lattice Boltzmann method. We reimplemented the test cases, mentioned in the paper, to study the method-coupling and its impact on accuracy.
\end{sciabstract}
...
\newpage
Hier folgen Informationen \"uber das Stylesheet. Sollten wir dann irgendwann l\"oschen, fand ich aber f\"urs erste ganz hilfreich.
