...
with
\begin{equation}
	S^{(k)} = -\frac{3}{2\tau h^2} \sum_i c_i \otimes c_i(f_i-f_i^{eq})(t,x_k)
\end{equation}
\begin{equation}
...
\end{equation}
We use now the information of the level set method for the new boundary condition. What is missing now, is a refill method. Due to the fact that the interface moves, some cells change their fluid type. To do so we use a equilibrium/non-equilibrium refill (CITE). This refill procedure consists out of four steps:
\begin{itemize}
...
\end{itemize}
Suppose we choose an inward pointing direction $c_i$ in a boundary point $x = x_b \in \Omega_1$, e.g. the direction of smallest angle with the surface normal. Then the density is interpolated in the following way:
\begin{equation}
	\tilde{p} = 3\rho(x-c_i,t+1) - 3\rho(x-2c_i,t+1) + \rho(x-3c_i,t+1) \text{.}
\end{equation}
To interpolate u we choose ....
The same procedure has to be used in the second fluid $\Omega_2$.

TODO: Explain the refill method. Show Fig.2 of the Th\"ommes-paper and explain the coupling of the two methods.
