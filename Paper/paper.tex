...
The incompressible two-phase flow problem can be described mathematically with the Navier-Stokes equations for two incompressible fluids (\cref{nse1,nse2}). 
...
\end{equation}
and thus, all variables for the boundary conditions are defined. % sagen wir hier noch, dass im Thömmes-paper ein Fehler war, der im zweiten Paper korrigiert wurde?
...
Our Couette flow scenario does not account for the influence of surface tension as the fluid interface is not curved. If it would get distorded, surface tension would have a stabilizing effect although. 
Hence, we use the Young-Laplace law for the pressure drop across curved surfaces, to validate the implementation of surface tension. The pressure inside of a convex surfaces is higher than on the outside and the pressure drop $\Delta p$ of a bubble of radius $r$ is proportional to the surface tension coefficient $\sigma$ according to thefollowing relationship,
\begin{equation}
...
We want to thank Th\"ommes et al. for the awesome paper. % oder so ähnlich
Furthermore, we want to thank Ian Mitchell for providing the toolbox for level set methods.
