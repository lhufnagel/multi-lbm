...
\end{equation}
Here, $v_i$ is the velocity, $p_i$ the pressure, $v_i$ the kinematic viscosity and $\varrho_i$ the mass density.
...
\end{equation}
Here, T is the stress tensor, $\sigma$ the surface tension, $\kappa = \frac{1}{2}(\kappa_1 + \kappa_2)$ the mean curvature with respect to the surface normal $n$. Brackets denote the jump of a quantity: $[q](x) = \lim_{\epsilon \rightarrow 0} (q(x + \epsilon n) - (q(x - \epsilon n))$.
...
This paper is organized as follows: Section 2 gives a short introduction to the numerical methods and the coupling of these. In section 3 we present our results and finally in section 4 we draw a conclusion.

\section{Numerical Methods}
The method contains two different numerical methods. Before we show how these can be coupled, we will shortly introduce the numerical methods independently.

\subsection{The Lattice Boltzmann Method}
To solve the incompressible Naver-Stokes equations, we use the lattice Boltzmann method. This method consists of two steps. The streaming step and the collision step.
\begin{equation}
	\rho(x,t) = \sum_{i=0}^{8} f_i(x,t) \text{, } 
	u = \sum_{i=0}^{8} f_i(x,t)c_i
\end{equation}
\begin{equation}
	f_i(x+c_i,t+1) = f_i(x,t) - \omega(f_i-f_i^{eq}) + G_i
\end{equation}
\begin{equation}
...
\begin{equation}
	R_i = 6h^2f_i^* \cdot (-q(1-q)\Lambda_i:[S] - (q-\frac12)\Lambda_i:S^{(2)} \text{,}
\end{equation}
with
\begin{equation}
	S^{(k)} = -\frac{3}{2\tau h^2} \sum_i c_i \otimes c_i(f_i-f_i^{eq})(t,x_k) \text{,}
\end{equation}
\begin{equation}
...
\begin{equation}
	[p] \approx \frac{1}{3 h^2} (\rho(x_1,t)\varrho^{(1)} - \rho(x_2,t)\varrho^{(2)}) \text{.}
\end{equation}
We use now the information of the level set method for the new boundary condition. What is missing now, is a refill method. Due to the fact that the interface moves, some cells change their fluid type. We use a equilibrium/non-equilibrium refill (CITE) that consists out of four steps:
\begin{itemize}
...
\end{itemize}
Suppose we choose an inward pointing direction $c_i$ in a boundary point $x = x_b \in \Omega_1$, e.g. the direction of smallest angle with the surface normal. Then the density is interpolated by
\begin{equation}
	\tilde{\rho} = 3\rho(x-c_i,t+1) - 3\rho(x-2c_i,t+1) + \rho(x-3c_i,t+1) \text{.}
\end{equation}
In \cite{Thoemmes} the velocity is interpolated by using the velocity of the interface. The Ian-Mitchell toolbox does not provide a function to access this value. Therefore, we interpolate the velocity in the same way as we did for the density:
\begin{equation}
...
\begin{itemize}
	...
	\item[4.] Pass the current velocity field to the level set code and run it for the same number of steps (see Fig. MISSING)
