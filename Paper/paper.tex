...
%TODO: Hier fehlen Referenzen! ausserdem muessen noch die Gleichungen unten gelabelt werden!!!
...
\begin{equation}
	\rho(\vec{x},t) = \sum_{i=1}^{9} f_i(\vec{x},t) \text{, } 
...
\end{equation}
The parameter $\tau$ is the relaxation parameter for the BGK collision operator and controls the kinematic viscosity in lattice units $\nu = \frac16(2\tau - 1)$. Note, that we assumed the cell spacing $\Delta x$ and the time step $\Delta t$ to be unity here. The lattice speed of sound equals to $c_s =\frac{1}{\sqrt{3}}$. $G_i$ is an additional force like gravity. In our case we set $G_i = 0$. Furthermore, we use the equilibrium distribution
\begin{equation}
...
\end{itemize}
% TODO Propagation kurz im Text erklären. vlt Bounce-Back boundary conditions erklaeren?
...
For the coupling, we use a bounce back type boundary condition that prescribes the velocity as  Dirichlet value. For example, in Fig. \ref{coupling} for a point $x_2 \in \Omega_2$ that is next to the phase interface we set
\begin{figure}
...
\end{align}
Here $\mu_k = \varrho_k \nu_k$ are the dynamic viscosities, with $\bar{\mu} = (\mu_1 + \mu_2)/2$ and $\bar{S} = (S_1 + S_2)/2$ as averaged quantities, $\kappa$ the interface curvature, and the visocity jump  $[\mu] = \mu_2 - \mu_1$.
...
\begin{equation}
  \tilde{\rho}(x) = 3\rho(x+c_i,t+1) - 3\rho(x+2c_i,t+1) + \rho(x+3c_i,t+1) \text{.}
\end{equation}
\begin{figure}
...
Therefore, we follow the formula given in \cite{Lallemand} that is basically the same we use for density,
\begin{equation}
  \tilde{u}(x) = 3u(x+c_i,t+1) - 3u(x+2c_i,t+1) + u(x+3c_i,t+1) \text{.}
\end{equation}
