...
A more vivid possible application of the algorithm we presented is shown in \cref{cavity}, which shows the transient movement of an oil drop in a lid driven cavity. It is a known problem that level set methods tend to suffer from mass losses \cite{Thoemmes}, but still a very good insight into detailed flow behavior is possible. A variety of two-phase flow scenarios is possible for simulation with the presented method.
